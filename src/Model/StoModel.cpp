...
DSP_RTN_CODE StoModel::readSmps(const char * filename)
{
	...
	binCols = new list<int> [getNumStages()];
	intCols = new list<int> [getNumStages()];
	ctype_core_ = new char * [getNumStages()];

	/** collect binary and integer columns*/
	for (int stg = 0; stg < getNumStages(); ++stg)
	{
		CoinFillN(ctype_core_[stg], getNumCols(stg), 'C');
		for (int j = 0; j < getCore()->getBinaryLength(); ++j)
		{
			int binIndex = getCore()->getBinaryIndices()[j];
			if (binIndex < getColStart(stg) ||
				binIndex >= getColStart(stg) + getNumCols(stg))
				continue;
			binCols[stg].push_back(binIndex - getColStart(stg));
			ctype_core_[stg][binIndex - getColStart(stg)] = 'B';
		}
		for (int j = 0; j < getCore()->getIntegerLength(); ++j)
		{
			int intIndex = getCore()->getIntegerIndices()[j];
			if (intIndex < getColStart(stg) ||
				intIndex >= getColStart(stg) + getNumCols(stg))
				continue;
			intCols[stg].push_back(intIndex - getColStart(stg));
			ctype_core_[stg][intIndex - getColStart(stg)] = 'I';
		}
	}
		...
		int stg /**< stage */)
{
	...
	/** reserve memory */
	split->reserve(getNumCols(stg));

	/** insert elements */
	for (j = 0; j < rows_core_[i]->getNumElements(); ++j)
	{
		ind = rows_core_[i]->getIndices()[j] - getColStart(stg);
		if (ind >= 0 && ind < getNumCols(stg))
		{
...
{
	getCore()->copyColLower(clbd, stg);
	for (std::list<int>::const_iterator ci = binCols[stg].begin(); ci != binCols[stg].end(); ++ci)
	{
		clbd[*ci] = 0.0;
	}
}
...
{
	getCore()->copyColUpper(cubd, stg);
	for (std::list<int>::const_iterator ci = intCols[stg].begin(); ci != intCols[stg].end(); ++ci)
	{
		cubd[*ci] = 1.0;
	}
}
		...
		int node                /**< node index */)
{
	...
	for (j = 0; j < node_data_[node].mat_scen_->getVectorSize(i); ++j)
	{
		col = ind[j] - getColStart(stg);
		if (col >= 0 && col < getNumCols(stg))
		{
...
void StoModel::combineRandColLower(double * clbd, SmiScnNode* node)
{
	node->getNode()->copyColLower(clbd);
}

/** combine random column upper bounds */
void StoModel::combineRandColUpper(double * cubd, SmiScnNode* node)
{
	node->getNode()->copyColUpper(cubd);
}

/** combine random objective coefficients */
void StoModel::combineRandObjective(double * obj, SmiScnNode* node, bool adjustProbability)
{
	node->getNode()->copyObjective(obj);

	if (adjustProbability)
	{ 
		for (int j = getNumCols(node->getStage()) - 1; j >= 0; --j)
		{
			obj[j] *= node->getProb();
		}
	}
...
void StoModel::combineRandRowLower(double * rlbd, SmiScnNode* node)
{
	node->getNode()->copyRowLower(rlbd);
}

/** combine random row upper bounds */
void StoModel::combineRandRowUpper(double * rubd, SmiScnNode* node)
{
	node->getNode()->copyRowUpper(rubd);
}
