...
#include "Model/DecStoModel.h"

DecStoModel::DecStoModel() :
StoModel(),
...
/** copy constructor */
DecStoModel::DecStoModel(const DecStoModel & rhs) :
StoModel(rhs),
master_col_indices_(NULL) {
	/** nothing to do */
}
...
/** copy constructor */
DecStoModel::DecStoModel(const StoModel & rhs) :
StoModel(rhs),
master_col_indices_(NULL) {
	/** nothing to do */
}
...
bool DecStoModel::isDistributed()
{
...
DSP_RTN_CODE DecStoModel::decompose(
    SmiScenarioTree<SmiScnNode *> * subTree,    /**< [in] subproblem tree */
...
)
{
    ...
    int nrows = 0;
    int ncols = 0;
    for (std::vector<SmiScnNode *>::iterator it = subTree->treeBegin(); it != subTree->treeEnd(); ++it)
    {
        nrows += (*it)->getNumRows();
        ncols += (*it)->getNumCols();
    }
	...
	bool adjust_probability)
{
	...
	for (i = 0; i < nrows_[0]; ++i) {
		for (j = 0; j < getRowCore(i)->getNumElements(); ++j) {
			if (fabs(getRowCore(i)->getElements()[j]) > 1.e-10)
				nzcnt++;
		}
	}
	...
	for (s = 0; s < size; ++s)
	{
		if (fromSMPS_)
		{
			for (i = nrows_[0]; i < nrows_core_; ++i) {
				nzcnt += getRowCore(i)->getNumElements();
			}
		}
		else
	...
	for (i = 0; i < nrows_[0]; ++i)
	{
		for (j = 0; j < getRowCore(i)->getNumElements(); ++j) {
			if (fabs(getRowCore(i)->getElements()[j]) > 1.e-10) {
				...
				colIndices.push_back(getRowCore(i)->getIndices()[j]);
				elements.push_back(getRowCore(i)->getElements()[j]);
				pos++;
			}
		}
		rownum++;
	...
	for (s = 0; s < size; ++s)
	{
		for (i = nrows_[0]; i < nrows_core_; ++i)
		{
			...
			if (fromSMPS_)
			{
				...
				for (int j = 0; j < getRowCore(i)->getNumElements(); ++j)
					denserow[getRowCore(i)->getIndices()[j]] = getRowCore(i)->getElements()[j];
...
DSP_RTN_CODE DecStoModel::decomposeCoupling(
	int size,                    /**< [in] size of subproblem subset */
...
const int * DecStoModel::getSubproblemCouplingColIndices(int s) {
...
double DecStoModel::evalLhsCouplingRow(int row, double ** solutions)
{
	double val = 0;
...
double DecStoModel::evalLhsCouplingRowSubprob(int row, int s, double * subprobSolution)
{
...
double DecStoModel::evalLhsCouplingRowAlternative(int row, double ** solutions)
{
	double val = 0;
//	for (int s = 0; s < getNumSubproblems(); s++)
...
void DecStoModel::convertLagrangianFromAlternative(double * multipliers, double *& newMultipliers)
{
...
DSP_RTN_CODE DecStoModel::getFullModel(
	CoinPackedMatrix *& mat, /**< [out] constraint matrix */
	double *& clbd,          /**< [out] column lower bounds */
	double *& cubd,          /**< [out] column upper bounds */
	char   *& ctype,         /**< [out] column types */
...
DSP_RTN_CODE DecStoModel::copySubprob(
...
DSP_RTN_CODE DecStoModel::copyRecoProb(
