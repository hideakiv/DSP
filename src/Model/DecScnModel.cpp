...
#include "DecScnModel.h"
#include "CoinHelperFunctions.hpp"


DecScnModel::DecScnModel()
{

}

/** copy constructor */
DecScnModel::DecScnModel(const DecScnModel & rhs)
{

}

DecScnModel::DecScnModel(const SmiScnModel & rhs)
{
...
DSP_RTN_CODE DecScnModel::readSmps(const char * filename)
{
	// int i, j, s;
	...
	int smistatus = SmiScnModel::readSmps(filename);
	if (smistatus < 0)
	{
    ...
    /** number of nodes */
    nnodes_ = getSmiTree()->wholeTree().size();

	/** number of scenarios */
	nscen_ = getNumScenarios();
	...
	nints_core_ = getCore()->getBinaryLength() + getCore()->getIntegerLength();

	for (int i = 0; i < nstgs_; ++i)
	{
		...
		sd.nrows_ = core->getNumRows(i);
		sd.ncols_ = core->getNumCols(i);
		sd.nints_ = (int) core->getIntCols(i).size();
		sd.rstart_ = core->getRowStart(i);
		sd.cstart_ = core->getColStart(i);
		...
		core->copyColLower(sd.clbd_core_, i);
		core->copyColUpper(sd.cubd_core_, i);
		core->copyObjective(sd.obj_core_, i);
		core->copyRowLower(sd.rlbd_core_, i);
		core->copyRowUpper(sd.rubd_core_, i);
	...
	{
		node = core->getNode(s);
		int rstart_ = core->getRowStart(s);
		int nrows_  = core->getNumRows(s);
		for (int i = rstart_; i < rstart_ + nrows_; ++i)
		{
			...
			if (i != k)
			{
				CoinError("Unexpected core file structure.", "readSmps", "DecScnModel");
			}
			rows_core_[k++] = new CoinPackedVector(
					node->getRowLength(i),
	...
	for (int i = 0; i < nnodes_; ++i)
	{
		...
		SmiScnNode* scnnode = getSmiTree()->wholeTree()[i];
		/** get stage corresponding to scenario */
		nd.stg_ = scnnode->getStage();

		/** add mapping */
		/**
		 * TODO: assert that leafnodes are nstgs_-1
		 */
		if (nd.stg_ == nstgs_-1)
		{
			scen2node_.insert(std::pair<int,int>(i,stg));
		}
		...
		lens.resize(nrows_);
		for (int j = rstart_; j < rstart_ + nrows_; ++j)
			lens[j-rstart_] = node->getRowLength(j);
		nd.mat_scen_  = new CoinPackedMatrix(false, ncols_, nrows_, node->getNumMatrixElements(), 
				node->getRowElements(rstart_), node->getRowIndices(rstart_), node->getRowStarts(rstart_), &lens[0]);
		node_data_[i] = nd;
	}
...
const CoinPackedVector * DecScnModel::getObjScenario(int scenario, int stage) 
{
...
bool DecScnModel::mapVarnameIndex(map<string, int> &map_varName_index, const char * smps) 
{
	char core[128];
...
void DecScnModel::setProbability(double *probability)
{
	CoinCopyN(probability, nscen_, prob_);
}

void DecScnModel::setSolution(int size, double * solution)
{
	if (size > 0)
		init_solutions_.push_back(new CoinPackedVector(size, solution));
}

// DSP_RTN_CODE DecScnModel::setWassersteinAmbiguitySet(double lp_norm, double eps)
// {
// 	if (nstgs_ == 0)
// 	{
...
// void DecScnModel::normalizeProbability()
// {
// 	double prob_sum = 0.0;
// 	for (int s = 0; s < nscen_; ++s)
// 		prob_sum += prob_[s];
...
CoinPackedVector * DecScnModel::splitCoreRowVec(
		int i,  /**< row index */
		int stg /**< stage */)
{
	int j, ind;
...
// void DecScnModel::copyCoreQuadraticObjective(CoinPackedMatrix *&qobj_coupling, CoinPackedMatrix *&qobj_ncoupling, int stg)
// {
// 	vector<int> colidx;
// 	vector<int> rowidx;
...
void DecScnModel::combineRandColLower(double * clbd, int stg, int node)
{
...
void DecScnModel::combineRandColUpper(double * cubd, int stg, int node)
{
...
void DecScnModel::combineRandObjective(double * obj, int stg, int node, bool adjustProbability)
{
	int i, j;
...
// void DecScnModel::combineRandQuadraticObjective(CoinPackedMatrix * &qobj_coupling, CoinPackedMatrix * &qobj_ncoupling, int stg, int scen, bool adjustProbability)
// {
// 	int i, j, s;
...
void DecScnModel::combineRandRowLower(double * rlbd, int stg, int node)
{
...
void DecScnModel::combineRandRowUpper(double * rubd, int stg, int node)
{
...
void DecScnModel::shiftVecIndices(
...
void DecScnModel::addBranchingHyperplane(int nzcnt, int * indices, double * values, int priority)
{
...
void DecScnModel::__printData()
{
	char tmpstr[128];

	printf("\n### BEGINNING of printing DecScnModel data ###\n\n");
...
//		PRINT_COIN_PACKED_VECTOR_MSG((*rubd_scen_[s]), "rubd_scen_")
	...
	printf("\n### END of printing DecScnModel data ###\n");
}

// DSP_RTN_CODE DecScnModel::printQuadRows(const int s)
// {
// 	BGN_TRY_CATCH
...
// DSP_RTN_CODE DecScnModel::printQuadRows(const QuadRowData *qc)
// {
// 	BGN_TRY_CATCH
...
// double DecScnModel::getWassersteinDist(int i, int j) {
...
// double DecScnModel::getReferenceProbability(int i) {
